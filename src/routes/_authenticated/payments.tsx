...
function PaymentsPage() {
    ...
    return payments.filter((p) => {
      ...
      if (selectedMonth) {
        ...
        if (
          ...
          paymentDate.getMonth() !== selectedMonth.month
        ) {
          return false;
        }
      }
  ...
  useEffect(() => {
    setPage(1);
  }, [search, method, selectedMonth]);

  const effectivePerPage = itemsPerPage === 0 ? filtered.length || 1 : itemsPerPage;
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-3 gap-3 text-xs sm:text-sm px-1">
        <div className="flex items-center gap-3">
          <div className="text-muted-foreground">{filtered.length} payments</div>
          <select

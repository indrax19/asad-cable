...
import { collection, onSnapshot, query, where } from "@/lib/supabase-store";
...
function ReportsPage() {
  ...
  const effectivePerPage = itemsPerPage === 0 ? rows.length || 1 : itemsPerPage;
  const totalPages = Math.max(1, Math.ceil(rows.length / effectivePerPage));
  const currentPage = Math.min(page, totalPages);
  const pageRows = rows.slice((currentPage - 1) * effectivePerPage, currentPage * effectivePerPage);
    ...
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-3 gap-3 text-xs sm:text-sm px-1">
        <div className="flex items-center gap-3">
          <div className="text-muted-foreground">{rows.length} records</div>
          <select

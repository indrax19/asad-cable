...
import { sendPasswordResetEmail, createManagedUser, updateManagedUser, deleteManagedUser } from "@/lib/supabase-auth";
    ...
    try {
      if (initial) {
        ...
        if (password) {
          ...
          try {
            await updateManagedUser(initial.uid, updatePayload, email, password);
          } catch (err) {
        ...
        const result = await createManagedUser(email, password, profile);
        // NO REDUNDANT setDoc - Edge Function already wrote it!
        // Just update area assignments
        for (const id of assignedAreaIds) {
          await updateDoc(doc(db, "areas", id), { dealerIds: arrayUnion(result.user.uid) });
        }
...
function ResetPasswordDialog({ dealer, onClose }: { dealer: UserDoc; onClose: () => void }) {
  ...
  const handleResetPassword = async () => {
    setBusy(true);
    try {
      await sendPasswordResetEmail(dealer.email!);
